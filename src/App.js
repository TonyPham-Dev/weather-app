...
function App() {
  ...
  useEffect(() => {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${saveValue || VALUE_DEFAULT}&appid=${API_KEY}&lang=vi`)
      ...
      .then(async dataWeather => {
        ...
        setSunrise(moment(dataWeather.sys.sunrise).format('H:mm'))
        setSunSet(moment(dataWeather.sys.sunset).format('H:mm'))
        setHumidity(dataWeather.main.humidity)
        setWind((dataWeather.wind.speed * 3.6).toFixed(1))

        setData(await dataWeather)
      })
      .catch(err => {
        console.log(err)
      })
  ...
  return (
      <div className="container">
          <div className="content-Top">
            ...
            />
            <button onClick={handleSearch} type="submit" className="btn_search">Search</button>
          </div>

          <div className="parameter">
            <h4>{data.name || DEFAULT_VALUE}</h4>
            <p style={{fontSize:'20px', color:'#333', fontWeight:'bold'}}>{sky || DEFAULT_VALUE}</p> 
            
            <img className="" src={`http://openweathermap.org/img/wn/${icon}@2x.png` || DEFAULT_ICON}/>
